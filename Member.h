...
public:
    ...
    Address(string address) {
        ...
        memcpy(&addr[0], &id, sizeof(int)); // 4bytes for an int id, each addr bucket can hold 1 byte
        memcpy(&addr[4], &port, sizeof(short)); // 2 bytes for a short port
    }
 ...
 * This is one row of data on membership table
 */
