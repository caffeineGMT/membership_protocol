...
int main(int argc, char *argv[]) {
	...
	Application *app = new Application(argv[1]); // TODO: what is argv[1]?
...
void Application::mp1Run() {
	...
	for( i = 0; i <= par->EN_GPSZ-1; i++) {
		 ...
		 * Every single time unit, each node who are already initialized and still alive would enqueue a message
		 */
	...
	for( i = par->EN_GPSZ - 1; i >= 0; i-- ) {
		...
		if( par->getcurrtime() == (int)(par->STEP_RATE*i) ) {
			...
			nodeCount += i; // TODO: why += i? 0 + 1 + 2 + ... + n ?
		}
		 ...
		 */
			...
			mp1[i]->nodeLoop();
			#ifdef DEBUGLOG
			if( (i == 0) && (par->globaltime % 500 == 0) ) { // log every 500 time units
...
void Application::fail() {
	...
	else if( par->getcurrtime() == 100 ) { // fail half of the node in multi node failure mode
...
Address Application::getjoinaddr(void){
    ...
    Address joinaddr;
    joinaddr.init();
    *(int *)(&(joinaddr.addr))=1; // TODO: what are the addr[0] thru addr[4]
